...
if (node.contextInfo && (!predicate || predicate(node.contextInfo))) return node.contextInfo
if (node.message && typeof node.message === 'object') {
const found = getMessageContextInfo(node.message, predicate)
if (found) return found
}
for (const value of Object.values(node)) {
if (!value || typeof value !== 'object') continue
const found = getMessageContextInfo(value, predicate)
if (found) return found
}
return null
}

function getMessageMentionedJid(message, msg = null) {
const direct = message?.extendedTextMessage?.contextInfo?.mentionedJid
if (Array.isArray(direct) && direct.length) return direct
const sources = [msg, message]
for (const source of sources) {
const contextInfo = getMessageContextInfo(source, info => Array.isArray(info?.mentionedJid) && info.mentionedJid.length)
if (contextInfo) return contextInfo.mentionedJid
}
return []
}

function extractButtonResponseText(msg = {}) {
...
return params.id || params.selectedId || params.rowId || params.button_id || params.display_text || params.title || ''
} catch {}
}
return nativeFlow?.name || interactive?.body?.text || ''
}
...
export class SimpleSocketService {
    static serializeMessage(conn, message, hasParent) {
        return smsg(conn, message, hasParent)
    }
...
export async function makeWASocket(connectionOptions, options = {}) {
    ...
    let sock = Object.defineProperties(conn, {
        ...
        decodeJid: {
            value(jid) {
                return SimpleSocketService.decodeJid(jid)
            }
        },
        logger: {
            get() {
    ...
    return conn.relayMessage(jid, prep.message, { messageId: prep.key.id })
        ...
        getFile: {
            ...
            async value(PATH, saveToFile = false) {
                let res, filename
                const data = Buffer.isBuffer(PATH) ? PATH : PATH instanceof ArrayBuffer ? PATH.toBuffer() : /^data:.*?\/.*?;base64,/i.test(PATH) ? Buffer.from(PATH.split`,`[1], 'base64') : /^https?:\/\//.test(PATH) ? Buffer.from(await (res = await fetch(PATH)).arrayBuffer()) : fs.existsSync(PATH) ? (filename = PATH, await fs.promises.readFile(PATH)) : typeof PATH === 'string' ? PATH : Buffer.alloc(0)
...
const storeContacts = conn.baileysStore?.contacts || store?.contacts || {}
const contact = conn.chats?.[jid] || storeContacts[jid] || Object.values(storeContacts).find((contact) => contact?.lid === jid || contact?.jid === jid) || {}
const contactName = (withoutContact ? '' : contact?.name) || contact?.subject || contact?.vname || contact?.notify || contact?.verifiedName
if (contactName) return contactName
return Promise.resolve(conn.signalRepository?.lidMapping?.getPNForLID?.(jid)).then((mapped) => {
const mappedJid = mapped ? conn.decodeJid?.(mapped) || mapped : jid
...
enumerable: true
        ...
        loadMessage: {
            value(messageID) {
                return conn.baileysStore?.loadMessage?.(messageID) || null
            },
            enumerable: true
        },
        sendGroupV4Invite: {
        ...
        insertAllGroup: {
            async value() {
                if (conn.baileysStore?.insertAllGroup) return conn.baileysStore.insertAllGroup()
                const groups = await getCachedParticipatingGroups(conn)
                    ...
                    try {
                        ...
                        if (message.message?.[mtype]?.contextInfo?.quotedMessage) {
                            ...
                            if ((remoteJid && remoteJid !== 'status@broadcast') && quoted) {
                                ...
                                if (!qChats.name) qChats.name = message.pushName || qChats.name || ''
                            }
                        }
                        ...
                        if (!fromMe && message.pushName && !isGroup) chats.name = message.pushName
                    } catch (e) {
                        console.error(e)
                    }
                }
        ...
        serializeM: {
             ...
             */
            value(m) {
                return SimpleSocketService.serializeMessage(conn, m)
            }
        },
...
export function serialize() {
    ...
    isBaileys: {
    get() {
    return (this?.fromMe || areJidsSameUser(this.conn?.user?.id, this.sender)) && (this.id?.startsWith('3EB0') ?? false) && (this.id?.length === 20 || this.id?.length === 22 || this.id?.length === 12) || false
        },
    },
        chat: {
            get() {
        ...
        quoted: {
            get() {
                ...
                const self = this
                const msg = self.msg
                const contextInfo = getMessageContextInfo(msg, info => Boolean(info?.quotedMessage))
                ...
                return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? { text: q } : q)), {
                    ...
                    mentionedJid: {
                        get() {
const mentionedJid = getMessageMentionedJid(quoted, q)
return mentionedJid.length ? mentionedJid : self.getQuotedObj()?.mentionedJid || []
                        },
                        enumerable: true
                    },
                    name: {
                    ...
                    download: {
                        value(saveToFile = false) {
                            ...
                            return mtype && this.mediaMessage?.[mtype] ? self.conn?.downloadM(this.mediaMessage[mtype], mtype.replace(/message/i, ''), saveToFile) : null
                        },
                        enumerable: true,
                        configurable: true,
                    },
                ...
                text: {
                        get() {
                if (!this || (!this.message && !this.msg)) return ''
                const msg = this.msg
                const buttonText = extractButtonResponseText(this.message)
                if (buttonText) return typeof this._text === 'string' ? this._text : buttonText
...
mentionedJid: {
get() {
return Array.isArray(this._mentionedJid) ? this._mentionedJid : getMessageMentionedJid(this.message, this.msg)
},
set(value) {
this._mentionedJid = Array.isArray(value) ? value : []
},
enumerable: true
},
        name: {
        ...
        download: {
            value(saveToFile = false) {
                const mtype = this.mediaType
                return mtype && this.mediaMessage?.[mtype] ? this.conn?.downloadM(this.mediaMessage[mtype], mtype.replace(/message/i, ''), saveToFile) : null
            },
            enumerable: true,
            configurable: true
        },
