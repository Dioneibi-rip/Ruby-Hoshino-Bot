...
await m.react('🌸')
}catch(error){console.error(error);return conn.reply(m.chat,`❌ 𝑬𝒓𝒓𝒐𝒓 𝒂𝒍 𝒅𝒆𝒔𝒄𝒂𝒓𝒈𝒂𝒓:\n${error.message}`,m)}
}
...
}catch(error){console.error(error);return conn.reply(data.chat,`❌ 𝑬𝒓𝒓𝒐𝒓 𝒂𝒍 𝒅𝒆𝒔𝒄𝒂𝒓𝒈𝒂𝒓:\n${error.message}`,m)}
})
