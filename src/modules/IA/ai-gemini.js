...
if (Array.isArray(node)) for (const x of node) walkDeep(x, visit, depth + 1, maxDepth)
else if (node && typeof node === 'object') for (const k of Object.keys(node)) walkDeep(node[k], visit, depth + 1, maxDepth)
}
...
const r = await fetch('https://gemini.google.com/_/BardChatUi/data/batchexecute?rpcids=maGuAc&source-path=%2F&hl=en-US&rt=c', { method: 'POST', redirect: 'manual', headers: { 'content-type': 'application/x-www-form-urlencoded;charset=UTF-8', 'user-agent': UA }, body: 'f.req=%5B%5B%5B%22maGuAc%22%2C%22%5B0%5D%22%2Cnull%2C%22generic%22%5D%5D%5D&' })
...
if (previousId) { try { resumeArray = JSON.parse(atob2(previousId))?.resumeArray || null } catch {} }
let lastErr = null
...
const response = await fetch('https://gemini.google.com/_/BardChatUi/data/assistant.lamda.BardFrontendService/StreamGenerate?hl=en-US&rt=c', { method: 'POST', headers: { 'content-type': 'application/x-www-form-urlencoded;charset=UTF-8', 'user-agent': UA, 'x-same-domain': '1', cookie }, body: new URLSearchParams(params) })
if (!response.ok) throw new Error(`${response.status} ${response.statusText}`)
const parsed = parseStream(await response.text())
return { text: parsed.text, id: btoa2(JSON.stringify({ resumeArray: parsed.resumeArray })), images: parsed.images }
} catch (e) { lastErr = e; if (attempt < 3) await new Promise(r => setTimeout(r, 700)) }
}
