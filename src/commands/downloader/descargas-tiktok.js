...
await m.react('🌸')
}catch(error){console.error(error);if(await replyIfMediaTooLarge(conn,m.chat,error,m,{label:'video de TikTok'}))return;return conn.reply(m.chat,`❌ 𝑬𝒓𝒓𝒐𝒓 𝒂𝒍 𝒅𝒆𝒔𝒄𝒂𝒓𝒈𝒂𝒓:\n${error.message}`,m)}
}
