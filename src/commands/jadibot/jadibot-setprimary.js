...
if (!raw) return ''
const candidate = raw.includes('@') ? raw : `${raw.replace(/[^0-9]/g, '')}@s.whatsapp.net`
const normalized = normalizeSessionJid(jidNormalizedUser(candidate) || candidate)
const number = normalized.split('@')[0]?.replace(/[^0-9]/g, '') || ''
return number ? `${number}@s.whatsapp.net` : ''
}

function uniqueJids(values = []) {
return [...new Set((Array.isArray(values) ? values : [values]).map(toWhatsAppJid).filter(Boolean))]
}
...
const firstPass = toWhatsAppJid(rawTarget)
const rawString = String(rawTarget || '').trim()
const participant = normalizedParticipants.find((p = {}) => {
const ids = [p.id, p.jid, p.lid].map(String)
const waIds = uniqueJids(ids)
return waIds.includes(firstPass) || ids.includes(rawString)
})
const source = participant?.jid || participant?.id || participant?.lid || rawTarget
const resolved = await normalizeIdentityJid(conn, source, participantsByLid)
const primaryJid = toWhatsAppJid(resolved || firstPass || source)
...
if (!primaryJid) return m.reply('⚠️ 𝘿𝙚𝙗𝙚𝙨 𝙢𝙚𝙣𝙘𝙞𝙤𝙣𝙖𝙧, 𝙧𝙚𝙨𝙥𝙤𝙣𝙙𝙚𝙧 𝙤 𝙚𝙨𝙘𝙧𝙞𝙗𝙞𝙧 𝙚𝙡 𝙣𝙪́𝙢𝙚𝙧𝙤 𝙙𝙚𝙡 𝙗𝙤𝙩 𝙦𝙪𝙚 𝙙𝙚𝙨𝙚𝙖𝙨 𝙚𝙨𝙩𝙖𝙗𝙡𝙚𝙘𝙚𝙧 𝙘𝙤𝙢𝙤 𝙥𝙧𝙞𝙣𝙘𝙞𝙥𝙖𝙡.')
const chat = global.db.getChat?.(m.chat) || global.db.get?.('chats', m.chat) || {}
if (isSamePrimary(chat, aliases)) return conn.reply(m.chat, `✨ @${primaryJid.split`@`[0]} 𝙮𝙖 𝙚𝙨 𝙚𝙡 𝙗𝙤𝙩 𝙥𝙧𝙞𝙢𝙖𝙧𝙞𝙤 𝙙𝙚 𝙚𝙨𝙩𝙚 𝙜𝙧𝙪𝙥𝙤.`, m, { mentions: [primaryJid] })
const nextChat = {
...chat,
