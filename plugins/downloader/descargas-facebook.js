...
var handler = async (m, { conn, args, command, usedPrefix, text }) => {
  ...
  async function reportError(e) {
    await conn.reply(m.chat, `⁖🧡꙰ 𝙾𝙲𝚄𝚁𝚁𝙸𝙾 𝚄𝙽 𝙴𝚁𝚁𝙾𝚁: ${e.message || e}`, m)
    console.error(e)
  }
  ...
  // Envolver fbdl con timeout manual
  async function fbdlWithTimeout(url, timeoutMs = 25000) {
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error('Tiempo de espera agotado al descargar')), timeoutMs)
      fbdl(url).then(result => {
        clearTimeout(timer)
        resolve(result)
      }).catch(err => {
        clearTimeout(timer)
  ...
  if (isCommand7) {
    if (!text) return conn.reply(m.chat, `🚩 *Ingrese un enlace de facebook*`, m)

    if (!args[0].match(/www.facebook.com|fb.watch|web.facebook.com|business.facebook.com|video.fb.com/g))
      return conn.reply(m.chat, '🚩 *No es un enlace válido de Facebook*', m)
    ...
    m.react('⏳')  // Usa un emoji directo, o tu variable rwait si existe

    try {
      // Llamada con timeout controlado
      const fb = await fbdlWithTimeout(args[0])
      
      if (!fb?.data?.length) throw new Error('No se obtuvo el video. Posible enlace privado o caído.')
