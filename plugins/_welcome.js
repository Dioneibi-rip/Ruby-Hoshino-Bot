...
export async function before(m, { conn, participants, groupMetadata }) {
    ...
    // Ampliamos la cobertura para asegurar que detecte invitaciones por link y otros métodos
    const isWelcome = [
    ...
    // Si entran por link o salen por su cuenta, messageStubParameters viene vacío. 
    // En ese caso, la persona que hizo la acción (m.sender) es nuestro usuario afectado.
    let usuariosAfectados = m.messageStubParameters && m.messageStubParameters.length > 0 
    ...
    for (let userId of usuariosAfectados) {
        if (!userId) continue; // Prevención en caso de datos vacíos
