...
// let pp = 'https://www.bighero6challenge.com/images/thumbs/Piedra,-papel-o-tijera-00033181584.jpeg'
  ...
  if (textm == astro) {
    global.db.addEconomy(m.sender, { exp: 500 });
    m.reply(`*${emoji2} Empate!*\n\n*👉🏻 Tu: ${textm}*\n*👉🏻 El Bot: ${astro}*\n*🎁 Premio +500 XP*`);
  } else if (text == 'papel') {
    if (astro == 'piedra') {
      global.db.addEconomy(m.sender, { exp: 1000 });
      m.reply(`*${emoji} Tú ganas! 🎉*\n\n*👉🏻 Tu: ${textm}*\n*👉🏻 El Bot: ${astro}*\n*🎁 Premio +1000 XP*`);
    } else {
      global.db.addEconomy(m.sender, { exp: -300 });
      m.reply(`*💀 Tú pierdes! ❌*\n\n*👉🏻 Tu: ${textm}*\n*👉🏻 El Bot: ${astro}*\n*❌ Premio -300 XP*`);
    }
  } else if (text == 'tijera') {
    if (astro == 'papel') {
  ...
  global.db.updateUser(m.sender, { wait: Date.now() });
};
handler.help = ['ppt'];
handler.tags = ['games'];
handler.command = ['ppt'];
