...
normalizeChatDefaults(chat = {}) { if (typeof chat.welcome === 'undefined') chat.welcome = true; if (typeof chat.antiLink === 'undefined') chat.antiLink = true; if (typeof chat.antilink === 'undefined') chat.antilink = true; if (typeof chat.detect === 'undefined') chat.detect = true; if (chat.isBanned === true) chat.isBanned = { '*': true }; else if (!chat.isBanned || typeof chat.isBanned !== 'object') chat.isBanned = {}; chat.bannedBots = Array.isArray(chat.bannedBots) ? chat.bannedBots : Object.entries(chat.isBanned).filter(([, value]) => value === true).map(([jid]) => jid); return chat }
