...
const rows = this.statements?.allJson ? this.statements.allJson.all(section) : this.sqlite.prepare('SELECT id,value FROM json_records WHERE section=?').all(section)
const st = this.sqlite.prepare(`INSERT INTO ${section}(id,value,updated_at) VALUES(?,?,unixepoch()) ON CONFLICT(id) DO NOTHING`)
const tx = this.sqlite.transaction(list => { for (const row of list) st.run(row.id, row.value || '{}') })
...
_bindPublicApi() {
for (const name of ['getUser', 'getGroup', 'upsertGroupMetadata', 'listGroups', 'updateUser', 'userExists', 'getChat', 'updateChat', 'listUsers', 'listUserRows', 'addMoney', 'addEconomy', 'setEconomy', 'incrementUserField', 'syncCharactersFts', 'searchCharacter', 'getSection', 'replaceSection', 'setMarriagePair', 'divorcePair', 'getMarriages', 'replaceMarriages', 'getHarem', 'replaceHarem', 'upsertHaremClaim', 'getGachaMarket', 'replaceGachaMarket', 'addGachaMarketSale', 'removeGachaMarketSale', 'get', 'set', 'has', 'delete', 'read', 'write', 'flush', 'scheduleFlush', 'save', 'close', 'snapshot']) {
...
getChat(id) { if (!id || typeof id !== 'string') throw new TypeError('getChat requiere un id de chat válido'); const row = this.sqlite.prepare('SELECT value FROM chats WHERE id=?').get(id); const chat = this.normalizeChatDefaults(parseJSON(row?.value, {})); this.set('chats', id, chat); return chat }
