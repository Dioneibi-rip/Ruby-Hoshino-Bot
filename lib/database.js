...
const marriageCols = this.sqlite.prepare('PRAGMA table_info(marriages)').all().map(col => col.name)
if (!marriageCols.includes('married_at')) {
try {
...
this.sqlite.prepare("ALTER TABLE marriages ADD COLUMN partner_id TEXT NOT NULL DEFAULT ''").run()
