...
_bindPublicApi() {
for (const name of ['getUser', 'updateUser', 'userExists', 'getChat', 'updateChat', 'listUsers', 'listUserRows', 'addMoney', 'addEconomy', 'setEconomy', 'incrementUserField', 'syncCharactersFts', 'searchCharacter', 'getSection', 'replaceSection', 'setMarriagePair', 'divorcePair', 'getMarriages', 'replaceMarriages', 'getHarem', 'replaceHarem', 'upsertHaremClaim', 'getGachaMarket', 'replaceGachaMarket', 'addGachaMarketSale', 'removeGachaMarketSale', 'get', 'set', 'has', 'delete', 'read', 'write', 'flush', 'scheduleFlush', 'save', 'close', 'snapshot']) {
...
normalizeChatDefaults(chat = {}) { if (typeof chat.welcome === 'undefined') chat.welcome = true; if (typeof chat.antiLink === 'undefined') chat.antiLink = true; if (typeof chat.antilink === 'undefined') chat.antilink = true; if (typeof chat.detect === 'undefined') chat.detect = true; if (!chat.botSettings || typeof chat.botSettings !== 'object') chat.botSettings = {}; if (chat.isBanned === true) chat.isBanned = { '*': true }; else if (!chat.isBanned || typeof chat.isBanned !== 'object') chat.isBanned = {}; for (const [jid, banned] of Object.entries(chat.isBanned)) { if (jid !== '*' && banned === true) { chat.botSettings[jid] ||= {}; if (typeof chat.botSettings[jid].isBanned === 'undefined') chat.botSettings[jid].isBanned = true } } chat.bannedBots = Object.entries(chat.botSettings).filter(([, value]) => value?.isBanned === true).map(([jid]) => jid); return chat }
getChat(id) { if (!id || typeof id !== 'string') throw new TypeError('getChat requiere un id de chat válido'); const chats = this.getSection('chats'); const chat = this.normalizeChatDefaults(chats[id] || {}); this.set('chats', id, chat); return chat }
