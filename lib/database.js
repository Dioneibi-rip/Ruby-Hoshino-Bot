...
export class SQLiteDatabase {
  constructor(filename = './src/database/database.sqlite', options = {}) {
    ...
    this.flushDelay = options.flushDelay || 250
  ...
  _markDirty(section, id) {
    ...
    if (this.autoFlush && !this._flushing && !this._closed) this._scheduleFlush()
  }
  _scheduleFlush() {
    if (this._flushTimer || this._closed) return
    this._flushTimer = setTimeout(() => {
      this._flushTimer = null
      try { this.flush() } catch (error) { console.error('[SQLiteDB] Error en flush diferido:', error) }
    }, this.flushDelay)
  ...
  close() { if (this._closed) return; clearInterval(this._timer); if (this._flushTimer) clearTimeout(this._flushTimer); this.flush(); this.sqlite.close(); this._closed = true }
}
