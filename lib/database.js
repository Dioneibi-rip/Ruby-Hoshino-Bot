...
`)
  ...
  userExists(id) { return Boolean(id && typeof id === 'string' && this.sqlite.prepare('SELECT 1 FROM users WHERE id=?').get(id)) }
  listUserRows() { return this.sqlite.prepare('SELECT * FROM users').all().map(row => this._rowToUser(row)) }
  listUsers() { const out = {}; for (const user of this.listUserRows()) out[user.id] = this.getUser(user.id); return out }
getUser(id) { if (!id || typeof id !== 'string') throw new TypeError('getUser requiere un id de usuario válido'); const row = this._rawUser(id) || this._createUser(id); return row ? this._userProxy(id) : {} }
  _userProxy(id) {
...
return Object.prototype.hasOwnProperty.call(user, prop) ? user[prop] : user.extras?.[prop]
  ...
  async read() { return this.data } async write() {} flush() {} close() { this.sqlite.close() } snapshot() { return { users: this.getSection('users'), marriages: this.getSection('marriages'), harem: this.getSection('harem'), claim_config: this.getSection('claim_config'), character_favorites: this.getSection('character_favorites') } }
}
