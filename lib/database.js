...
coin: { type: 'INTEGER', defaultSql: '0' }, bank: { type: 'INTEGER', defaultSql: '0' }, debt: { type: 'INTEGER', defaultSql: '0' }, exp: { type: 'INTEGER', defaultSql: '0' }, level: { type: 'INTEGER', defaultSql: '0' },
...
async read() { return this.data } async write() { this.flush() } flush() { const ids = [...this.dirtyUsers]; if (!ids.length) return; const tx = this.sqlite.transaction(rows => { for (const [id, user] of rows) this._writeUserRow(id, user) }); tx(ids.map(id => [id, this.userCache.get(id)]).filter(([, user]) => user)); for (const id of ids) this.dirtyUsers.delete(id) } close() { this.flush(); if (this.flushTimer) clearInterval(this.flushTimer); this.sqlite.close() } snapshot() { return { users: this.getSection('users'), marriages: this.getSection('marriages'), harem: this.getSection('harem'), gacha_market: this.getSection('gacha_market'), claim_config: this.getSection('claim_config'), character_favorites: this.getSection('character_favorites') } }
}
