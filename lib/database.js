...
const haremCols = this.sqlite.prepare('PRAGMA table_info(harem)').all().map(col => col.name)
if (!haremCols.includes('user_id')) {
const source = haremCols.includes('owner_jid') ? 'owner_jid' : haremCols.includes('id') ? 'id' : ''
if (source) {
this.sqlite.prepare("ALTER TABLE harem ADD COLUMN user_id TEXT NOT NULL DEFAULT ''").run()
this.sqlite.prepare(`UPDATE harem SET user_id=${q(source)} WHERE user_id=''`).run()
}
}
}
_prepareStatements() {
...
_bindPublicApi() {
for (const name of ['getUser', 'updateUser', 'userExists', 'getChat', 'updateChat', 'listUsers', 'listUserRows', 'addMoney', 'addEconomy', 'setEconomy', 'incrementUserField', 'getSection', 'replaceSection', 'setMarriagePair', 'divorcePair', 'getMarriages', 'replaceMarriages', 'getHarem', 'replaceHarem', 'upsertHaremClaim', 'get', 'set', 'has', 'delete', 'read', 'write', 'flush', 'close', 'snapshot']) {
...
const current = this.userCache.get(id) || this._rawUser(id) || this._createUser(id) || { id, extras: {} }
const next = { ...current, extras: { ...(current?.extras || {}) } }
...
delete(section, id) { if (section === 'users') { this.userCache.delete(id); this.userProxyCache.delete(id); this.dirtyUsers.delete(id); return this.sqlite.prepare('DELETE FROM users WHERE id=?').run(id) } this.sqlite.prepare('DELETE FROM json_records WHERE section=? AND id=?').run(section, id) }
...
async read() { return this.data } async write() { this.flush() } flush() { const ids = [...this.dirtyUsers]; if (!ids.length) return; const tx = this.sqlite.transaction(rows => { for (const [id, user] of rows) this._writeUserRow(id, user) }); tx(ids.map(id => [id, this.userCache.get(id)]).filter(([, user]) => user)); for (const id of ids) this.dirtyUsers.delete(id) } close() { this.flush(); if (this.flushTimer) clearInterval(this.flushTimer); this.sqlite.close() } snapshot() { return { users: this.getSection('users'), marriages: this.getSection('marriages'), harem: this.getSection('harem'), claim_config: this.getSection('claim_config'), character_favorites: this.getSection('character_favorites') } }
}
