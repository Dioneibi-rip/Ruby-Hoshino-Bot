...
async function loadCharacters({ force = false } = {}) {
  ...
  const stored = global.db?.getSection ? global.db.getSection('characters') : {}
  const parsed = Object.values(stored || {})
...
async function saveCharacters(characters) {
  ...
  if (!global.db?.replaceSection) throw new Error('SQLite no está inicializado para guardar personajes.')
  global.db.replaceSection('characters', Object.fromEntries(normalized.map((character, index) => [normalizeCharacterId(character.id || index), character])))
