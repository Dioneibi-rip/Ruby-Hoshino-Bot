...
const getRandomThumbnail = async (conn) => {
  ...
  try {
    ...
    if (file?.data && file?.mime?.startsWith('image/')) return file.data; // Retornamos solo el buffer directamente
  } catch (error) {
    console.error('Error descargando miniatura para externalAdReply:', error);
  }

  return null; // Si falla, enviamos null
};
...
const handler = async (type, conn, m, comando) => {
  ...
  if (msg) {
    ...
    const thumbBuffer = await getRandomThumbnail(conn);
    
    // Este es el enlace genérico de YouTube para "engañar" a WhatsApp
    ...
    const contextInfo = {
      ...
      externalAdReply: {
        title: packname,
        body: 'I🎀 𓈒꒰ 𝐘𝐚𝐲~ 𝐇𝐨𝐥𝐚𝐚𝐚! (≧∇≦)/',
        mediaType: 1, // Igual que en el play
        thumbnail: thumbBuffer, // Pasamos el buffer directamente
        renderLargerThumbnail: false,
        mediaUrl: youtubeTrickUrl, // <-- EL TRUCO ESTÁ AQUÍ
        sourceUrl: getSourceUrl() // Mantenemos tu canal para que si tocan, vayan ahí
      }
    };
