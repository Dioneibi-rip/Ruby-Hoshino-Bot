 ...
 */
// IMPORTANTE: Convertimos la función a async para poder usar 'await'
const handler = async (type, conn, m, comando) => {
  ...
  if (msg) {
    // 1. Obtenemos el link aleatorio de tu lista
    const imgUrl = getRandomIcono();
    let thumbBuffer = null;

    // 2. Descargamos la imagen antes de enviarla (¡La magia ocurre aquí!)
    try {
      thumbBuffer = (await conn.getFile(imgUrl))?.data;
    } catch (e) {
      console.error("Error al descargar la miniatura para el externalAdReply:", e);
    }
    ...
    const contextInfo = {
      ...
      externalAdReply: {
        title: packname,
        body: 'I🎀 𓈒꒰ 𝐘𝐚𝐲~ 𝐇𝐨𝐥𝐚𝐚𝐚! (≧∇≦)/',
        // 3. Pasamos el Buffer descargado en vez del link directo
        thumbnail: thumbBuffer, 
        // Nota: Asegúrate de que la variable 'redes' esté definida globalmente. Si no, pon un link de repuesto.
