...
const handler = async (type, conn, m, comando) => {
  // --- TUS TEXTOS ORIGINALES RESTAURADOS ---
  const msg = {
  ...
  if (msg) {
    ...
    try {
      ...
      const res = await conn.getFile(imgUrl);
      thumbBuffer = res.data;
    } catch (e) {
      console.log("Error al cargar miniatura");
    }

    // Usamos el enlace de YouTube como "mediaUrl" para forzar la visibilidad (el truco del play)
    // Pero el "sourceUrl" sigue siendo tu canal para cuando hagan click.
    ...
    const contextInfo = {
      ...
      externalAdReply: {
        ...
        mediaUrl: youtubeTrick, // Engaña a WhatsApp para mostrarlo
        sourceUrl: getSourceUrl() // Redirige a tu canal real
      }
    };

    // Al usar conn.reply, NO incluimos el link en el mensaje 'msg',
    // por lo que el link queda "oculto" pero la previsualización aparece arriba.
