      ...
      async value(jid, text = '', footer = '', text2 = '', messages, quoted, options) {
       ...
       // sendButton: {
            ...
            /*async value(jid, text = '', footer = '', buffer, buttons, quoted, options) {
                ...
                else if (buffer) try { (type = await conn.getFile(buffer), buffer = type.data) } catch (e) { buffer = null }
    ...
    conn.sendMessage(jid, { video: gif, gifPlayback: true, gifAttribution: b, caption: text, footer: footer, jpegThumbnail: file, templateButtons: but, ...options })
        ...
        insertAllGroup: {
            async value(force = false) {
                ...
                if (!force && conn._lastInsertAllGroup && now - conn._lastInsertAllGroup < 60000) return conn.chats
